...
export function parseGithubUrl(url) {
    const match = /^https:\/\/github\.com\/([^/]+)\/([^/]+)\/blob\/([^/]+)\/(.+?)(?:#L(\d+)(?:-L(\d+))?)?$/.exec(url);
    if (!match) {
        return null;
    }
    return {
...
function parseYoutubeStartTime(t) {
    ...
    const match = /^(?:(?<hours>\d+)h)?(?:(?<minutes>\d+)m)?(?:(?<seconds>\d+)s)?$/.exec(t);
    if (!match || match[0] === '') {
        return undefined;
    }
    const { hours, minutes, seconds } = match.groups;
    const total = Number(hours ?? 0) * 3600 +
        Number(minutes ?? 0) * 60 +
        Number(seconds ?? 0);
    return String(total);
}
 ...
 */
export function extractYoutubeVideoParameters(youtubeUrl) {
